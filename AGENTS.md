...
- **`.simplex/outbound` is deliberately `1777` — don't "fix" it.** `main.ts` chmods it (`OUTBOUND_MODE`) on every start. This container runs as root but consumers stage files as their own uid, which the bridge can't know and which two consumers needn't share, so widening the mode is the only option a chown can't cover. `.simplex` itself stays `0700`: dependency mounts are resolved by StartOS, so a consumer never traverses it. Rationale and limits in README's file-exchange section.
